...
const char * const EXPOSITIO_CONF_FILENAME = "expositio.conf";
const char * const EXPOSITIO_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/expositio/expositio/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //expositio-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", EXPOSITIO_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "expositio";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Expositio
    // macOS: ~/Library/Application Support/Expositio
    // Unix-like: ~/.expositio
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Expositio";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Expositio";
#else
    // Unix-like
    return pathRet / ".expositio";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", EXPOSITIO_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Expositio Core copyright is not removed by accident
    if (copyright_devs.find("Expositio Core") == std::string::npos) {
