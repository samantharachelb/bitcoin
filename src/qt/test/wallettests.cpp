...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<ExpositioAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_expositio-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_expositio-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_expositio-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = ExpositioUnits::formatWithUnit(unit, balance, false, ExpositioUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    ExpositioAmountField* amountInput = receiveCoinsDialog.findChild<ExpositioAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("expositio:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_expositio-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
