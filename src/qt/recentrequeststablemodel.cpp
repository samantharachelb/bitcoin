    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return ExpositioUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, ExpositioUnits::SeparatorStyle::NEVER);
            else
                return ExpositioUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+ExpositioUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
