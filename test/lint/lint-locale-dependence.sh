...
# Be aware that expositiod and expositio-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in expositiod.
#
...
# C locale.
#
# In contrast, expositiod does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where expositiod is guaranteed to be running
# with the classic locale ("C") whereas the locale of expositio-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in expositiod but
# "1,230000" in expositio-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/expositio-tx.cpp.*stoul"
    "src/expositio-tx.cpp.*trim_right"
