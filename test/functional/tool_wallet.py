...
class ToolWalletTest(ExpositioTestFramework):
    ...
    def expositio_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/expositio-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-chain=%s' % self.chain] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.expositio_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `expositio-wallet help` raises an error. Use `expositio-wallet -help`.
        ...
        if self.options.descriptors:
            error = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another expositiod?"
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/expositio/expositio/issues/7463
