...
The Expositio Core daemon can be configured to act as a trusted "border
router", implementing the expositio wire protocol and relay, making
...
The ZeroMQ feature in Expositio Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building expositiod:
...
For instance:
    ...
    $ expositiod -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/expositiod.tx.raw \
...
These options can also be provided in expositio.conf.
...
From the perspective of expositiod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into expositiod directly. Furthermore, no information is
...
using. Expositiod appends an up-counting sequence number to each
