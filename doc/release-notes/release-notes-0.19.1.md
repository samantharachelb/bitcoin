...
Expositio Core version 0.19.1 is now available from:

  <https://expositiocore.org/bin/expositio-core-0.19.1/>
...
From Expositio Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as Expositio Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, Expositio Core does not yet change appearance when
...
- #17488 fix "expositiod already running" warnings on macOS (fanquake)
